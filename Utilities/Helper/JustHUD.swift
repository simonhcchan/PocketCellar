...
class JustHUD: UIView {
    ...
    private var backColor = UIColor.black.withAlphaComponent(0.75)
    ...
    func showInView(view: UIView, withHeader: String?, andFooter: String?) {
        ...
        if let footer = andFooter, !isCleanedStringEmpty(string: footer) {
            ...
            footerLabel!.frame = CGRect(x: 0, y: 0, width: getFooterSize(text: footer).width, height: getLabelSize().height)
        }
    ...
    private func setBackGround(view: UIView) {
        ...
        let backFrame = getBackGroundFrame(view: self)
        backView = UIView(frame: backFrame)
        backView?.backgroundColor = UIColor.clear
        let translucentView = UIView(frame: backView!.bounds)
        translucentView.backgroundColor = backColor
        translucentView.alpha = 1//0.85
        ...
        translucentView.topAnchor.constraint(equalTo: backView!.topAnchor).isActive = true
        translucentView.leadingAnchor.constraint(equalTo: backView!.leadingAnchor).isActive = true
        translucentView.bottomAnchor.constraint(equalTo: backView!.bottomAnchor).isActive = true
        translucentView.trailingAnchor.constraint(equalTo: backView!.trailingAnchor).isActive = true
        
        backView?.layer.cornerRadius = 15.0
        backView?.clipsToBounds = true
        self.addSubview(backView!)
        self.backView!.translatesAutoresizingMaskIntoConstraints = false
        self.backView!.widthAnchor.constraint(equalToConstant: backFrame.width).isActive = true
        self.backView!.heightAnchor.constraint(equalToConstant: backFrame.height).isActive = true
        self.backView!.centerXAnchor.constraint(equalTo: self.centerXAnchor).isActive = true
    ...
    private func setFooterLabel(text: String) {
        ...
        footerLabel?.font = UIFont(name: "Lato-Regular" ,size: 13.0)
    ...
    private func getLabelSize() -> CGSize {
        ...
        let width = progressIndicator!.frame.width * 1.4
        let height = progressIndicator!.frame.height
        return CGSize(width: width, height: height)
    }//
    private func getFooterSize(text:String) -> CGSize {
        let size = text.size(withAttributes: [
            NSAttributedString.Key.font : UIFont.systemFont(ofSize: 15)
                ])
        let width = max(size.width, progressIndicator!.frame.width*1.3)
        let height = progressIndicator!.frame.height
        return CGSize(width: width, height: height)
    }
    // MARK: -Get Origin
